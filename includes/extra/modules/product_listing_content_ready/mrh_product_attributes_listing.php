 ...
 * @fix 2026-04-21 Phase 1 Non-Seeds: Kategorie-basierte Auto-Erkennung, Mini-Table fuer Non-Seeds unterdrueckt
 */
...
if (class_exists('MrhProductAttributes') && isset($module_content) && is_array($module_content)) {
    ...
    foreach ($mrh_pa_non_seed_paths as $mrh_pa_nsp) {
        if (strpos($mrh_pa_current_path, $mrh_pa_nsp) !== false) {
            $mrh_pa_is_non_seed_category = true;
            break;
        }
    }
    ...
    foreach ($module_content as &$mrh_pa_item) {
        ...
        // Phase 1 Non-Seeds: If product not in DB and we're in a non-seed category,
        // skip mini-table entirely (no Sorte/THC/CBD for Duenger, Grow, Headshop etc.)
        if (!$mrh_pa_attrs && $mrh_pa_is_non_seed_category) {
            $mrh_pa_item['MRH_MINI_TABLE'] = '';
            $mrh_pa_item['MRH_IS_SEED'] = false;
