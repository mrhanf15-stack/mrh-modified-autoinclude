    ...
    if (isset($payload['pictos']) && is_array($payload['pictos'])) {
        ...
        foreach ($payload['pictos'] as $p) {
            if (!empty($p['icon'])) {
                ...
                // Badge background and border colors (v1.1.1)
                if (!empty($p['bgcolor'])) {
                    $picto_data['bgcolor'] = preg_replace('/[^a-zA-Z0-9#]/', '', $p['bgcolor']);
                }
                if (!empty($p['bordercolor'])) {
                    $picto_data['bordercolor'] = preg_replace('/[^a-zA-Z0-9#]/', '', $p['bordercolor']);
                }
                $pictos[] = $picto_data;
            }
        }
